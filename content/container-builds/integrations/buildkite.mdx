...
The easiest option is to use a [Buildkite OIDC token](https://buildkite.com/docs/agent/cli/reference/oidc) as authentication for `depot build`. Our CLI supports authentication via OIDC by default in Buildkite when you have a trust relationship configured for your project.
