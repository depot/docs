...
<NoteCallout>
  **Looking for faster Python builds?** We recommend using [<u>uv</u>
  ](/docs/container-builds/optimal-dockerfiles/python-uv-dockerfile) for significantly faster dependency installation
  and better caching. uv supports Poetry projects natively and can speed up your builds by 10-100x while maintaining
  full compatibility with your `pyproject.toml` and `poetry.lock` files.
</NoteCallout>
