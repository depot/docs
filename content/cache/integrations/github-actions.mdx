...
For example, you can configure NX to use the GitHub Actions cache by using the [`@nx/shared-fs-cache`](https://nx.dev/docs/reference/deprecated/self-hosted-cache-packages) package combined with `actions/cache` to save and restore the `.nx/cache` directory.
