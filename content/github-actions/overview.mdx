...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <ul>
          <li>We scope cache entries stored in the Depot GitHub Actions cache by repository.</li>
          <li>Cache entries accessible only by the same repository that saved them.</li>
...
To estimate the potential cost savings by switching to Depot GitHub Action runners, enter in your current usage by runner type on our [GitHub Actions Price calculator](/github-actions-price-calculator).

### Additional usage pricing for GitHub Actions minutes

Startup and Business plans have the option to pay for additional GitHub Actions minutes on a per-minute basis. See the [runner type list](/docs/github-actions/runner-types) for the per-minute pricing for each runner type.
