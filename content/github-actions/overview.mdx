...
- **10x Faster cache:** Runners automatically integrate with our distributed cache architecture for upload and download speeds up to **1000 MiB/s** on **12.5 Gbps** of network throughput
...
- **No limits:** No concurrency limits, no cache size limits, no network limits, and no local disk size limits
