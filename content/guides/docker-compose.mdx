...
2. Buildx will create multiple Depot build requests for each image, so the Depot console will not display the `docker compose build` as a single unified request.
