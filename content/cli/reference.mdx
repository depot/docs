...
We've added support for lint for dockerfiles. In order to lint a dockerfile created with Depot's service, just use the `--lint` flag on the build file. You can also control the lint failure level with the added `--lint-on-fail` flag. By default Depot's lint flag will cause image build to fail on any lint errors.
...
We've added support for lint for dockerfiles. In order to lint a dockerfile created with Depot's service, just use the `--lint` flag on the build file. You can also control the lint failure level with the added `--lint-on-fail` flag. By default Depot's lint flag will cause builds to fail on any lint errors.
