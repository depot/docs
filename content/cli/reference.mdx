...
Interact with Depot projects. These are the new commands that allow you create and list projects, similar to the `depot list projects` command.

#### `depot projects create`

Crate a new project in your Depot organization.
...
| `organization` | Depot organization ID (required if in more than one organization and not using an [organization API token](/docs/api/authentication#generating-an-api-token)) |
...
| `token` | Depot token (if an [Organization API token](/docs/api/authentication#generating-an-api-token) is used, you don't need to specify a organization ID) |
{/* <!-- prettier-ignore-end --> */}
