---
title: 'Depot CLI: Container builds commands reference'
ogTitle: 'Depot CLI: Container builds commands reference'
description: Reference for all Depot container build CLI commands, including build, bake, cache, pull, push, and more.
---
...
Run any Cargo command with Depot Cache configured as the remote
[`sccache`](https://github.com/mozilla/sccache) backend. Install `sccache` and authenticate with `depot login` before
using this command.

All arguments other than `--org` are passed directly to Cargo:
...
If you belong to multiple organizations, select the cache namespace with `--org`:

```shell
depot cargo --org <organization-id> build --release
```
...
| `cache-policy-keep-days` | Build cache retention in days (default 14; 0 means no limit) |
| `token` | Depot token |
{/* <!-- prettier-ignore-end --> */}
...
Display the details and configuration of a Depot project.

If no project ID is provided, the command uses `DEPOT_PROJECT_ID` or the project configured in `depot.json` in the current
directory.
...
You can pass a project ID as an argument, and use `--output json` for machine-readable output:
...
Update the name, region, or cache storage policy of a Depot project. Only the values you pass as flags are changed, and at
least one of `--name`, `--region`, or `--cache-storage-policy` is required.
...
List images saved in a project's Depot Registry. In a terminal, the command opens an interactive table. When output
is redirected, it defaults to CSV.

```shell
depot image list --project <project-id>
depot image list --project <project-id> --output json
```

| Name      | Description                                     |
| --------- | ----------------------------------------------- |
| `output`  | Non-interactive output format (`json` or `csv`) |
| `project` | Depot project ID                                |
| `token`   | Depot API token                                 |

## `depot image rm`

Delete one or more images from the Depot Registry by tag. The aliases `remove` and `delete` are also available.

```shell
depot image rm --project <project-id> app:old app:staging
```
