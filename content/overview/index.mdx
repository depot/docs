...
description: Depot is a hosted Docker build service with blazing fast compute and persistent caching that can build container images up to 40x faster.
...
Depot is a remote container build service that can build Docker images up to 40x faster than building Docker images inside generic CI providers (with some projects showing [even faster speedups](https://depot.dev/benchmark/temporal)).
