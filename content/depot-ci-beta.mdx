...
Depot CI is free while in beta. Pricing on release:

- $0.0001 per second for base runner (2 CPUs and 8 GB memory)

We'll provide detailed pricing when Depot CI becomes generally available.
...
Secrets and variables can be organization-wide or scoped to a repository. Repository-scoped secrets override org-wide secrets with the same name.
...
You can also manage secrets and variables with the [Depot CI API](#configure-secrets-and-variables-with-the-api).

Currently, you can use the Depot CLI to manage only your org-wide [secrets](#manage-secrets-with-the-depot-cli) and [variables](#manage-variables-with-the-depot-cli).
...
| `jobs.<job_id>.permissions`           | Job-scoped permissions                | 🔨            | Done. With the exception of OIDC.                                                   |
...
We don't yet support reusing workflows from other repositories using the `uses` key, but we plan to support this in the future. You can still use the `uses` key to reference actions from the Marketplace, for example `uses: action/checkout@v1`.
...
5. Detects secrets and variables referenced in the selected workflows and prompts you to enter their values. Currently these imported secrets and variables are scoped org-wide.
...
If a workflow references a secret or variable that you haven't supplied via the `--secret` or `--var` flags, the migration completes with a warning. You can add missing secrets and variables later. See [Manage secrets and variables](#manage-secrets-and-variables).
...
Currently you can use the CLI to manage only org-level secrets. To manage all secrets, including repo-scoped secrets, see [Manage secrets and variables](#manage-secrets-and-variables).
...
Currently you can use the CLI to manage only org-level variables. To manage all variables, including repo-scoped variables, see [Manage secrets and variables](#manage-secrets-and-variables).
...
Before copying your workflows, [install the Depot Code Access app into your GitHub organization](#install-the-depot-code-access-app-into-your-github-organization).

### Copy your workflows to a Depot folder

For the beta, keep your GitHub Actions workflows running alongside Depot CI so that you don't end up blocked by a broken workflow.
...
You need an [organization token](/docs/api/authentication) (`DEPOT_TOKEN`) to authenticate. The v2 API supports both org-wide and repo-scoped secrets. Repo-scoped secrets override org-wide secrets with the same name.

To add an org-wide secret:

```bash
curl -X POST https://api.depot.dev/depot.ci.v2.SecretService/AddOrgSecret \
  -H "Authorization: Bearer ${DEPOT_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"name": "NPM_TOKEN", "value": "npm_abc123..."}'
```

To add a repo-scoped secret (format: `owner/name`):

```bash
curl -X POST https://api.depot.dev/depot.ci.v2.SecretService/AddRepoSecret \
  -H "Authorization: Bearer ${DEPOT_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"repo": "my-org/my-repo", "name": "DATABASE_URL", "value": "postgres://..."}'
```

To add multiple secrets at once:

```bash
curl -X POST https://api.depot.dev/depot.ci.v2.SecretService/BatchAddOrgSecrets \
  -H "Authorization: Bearer ${DEPOT_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"secrets": [{"name": "NPM_TOKEN", "value": "npm_abc123..."}, {"name": "DATABASE_PASSWORD", "value": "super-secret"}]}'
```

For the repo-scoped equivalent, use `BatchAddRepoSecrets` with the same body and add the `"repo": "owner/name"` field. Secret values can't be read back via the API after creation.

For variables, use `depot.ci.v2.VariableService` with the same org-scoped and repo-scoped pattern, or the [`depot ci vars` command](#manage-variables-with-the-depot-cli) (org-wide only). Variables are available as `${{ vars.VARIABLE_NAME }}` and can be read back, unlike secrets.
