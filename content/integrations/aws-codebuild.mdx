...
The CodeBuild Lambda compute type requires installing the `depot` CLI in a different directory that is in the `$PATH` by default,
...
When using the EC2 compute type in CodeBuild, you can login to your ECR registry with `docker login` via the [documented methods](https://docs.aws.amazon.com/codebuild/latest/userguide/sample-docker.html#sample-docker-files) provided by ECR. To access `docker login`, you must make sure that you're run CodeBuild environment is configured with Privileged mode turned on.
