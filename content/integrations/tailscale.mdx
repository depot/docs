...
Using Tailscale, Depot GitHub Actions runners, container builders, and CI jobs join your tailnet as [ephemeral nodes](https://tailscale.com/docs/features/ephemeral-nodes), and you can control their access to the rest of your infrastructure using Tailscale ACLs.
...
First, you will need to create a tag that will be assigned to all Depot runners. [Tailscale tags](https://tailscale.com/docs/features/tags) are used by Tailscale to group non-user devices, such as Depot runners, and let you manage access control policies based on these tags.
...
In your Tailscale [admin console](https://login.tailscale.com/admin/acls/file) access controls, [define a new tag under `tagOwners`](https://tailscale.com/docs/reference/syntax/policy-file#tag-owners):
...
Your Depot organization is now connected to your Tailscale tailnet. Depot runners, container builders, and CI jobs will now join your tailnet as [ephemeral nodes](https://tailscale.com/docs/features/ephemeral-nodes), using the tag you have created.

## Granting access to private services

Now that your Depot runners are connected to your tailnet, you can use Tailscale ACLs to control their access to the rest of your infrastructure. Depot runners will be [tagged](https://tailscale.com/docs/features/tags) with your chosen tag, which you can then reference in your ACL rules.

For example, you can grant your Depot runners access to a private database service by creating a new [ACL rule](https://tailscale.com/docs/reference/syntax/policy-file#access-rules) in the [admin console](https://login.tailscale.com/admin/acls/file):
...
Using [Tailscale subnet routers](https://tailscale.com/docs/features/subnet-routers), you can additionally grant your Depot runners access to entire subnets in any cloud provider VPC or on-premises network.
