...
If you leave `resources.diskGb` unset, the new sandbox gets the same disk size as the sandbox the snapshot was taken from. An explicit `diskGb` smaller than that size is rejected with `InvalidArgument`.
